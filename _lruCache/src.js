...
var LRUCacheItem = function (val, key) {
  var pair = [key, val];
  //add a new list node to the front of the list
  var node = this.list.unshift(pair)

  //add the new list node into the hashtable as key value pair
...
LRUCache.prototype.set = function (key, val) {
  //create a new LRUCacheItem
  LRUCacheItem.call(this, val, key);
  //if the size is greater than the limit
  if (this.length > this.limit) {
    //pop the list
    var pair = this.list.pop();

    //remove the list item from the hash table
    this.hashtable[pair[0]] = null;
    //decrease the size
    this.length--;
  }
};
