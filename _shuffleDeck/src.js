...
//output an array of suffled strings
//input an array of strings
//constraints try for linear time and constant space complexity
//edge cases an empty deck input would result in an empty deck output
var shuffleDeck = function(deck) {
  //set unshuffled to the length of the deck
  var unshuffled = deck.length;
  var random = 0;
  var shuffling = "";

  //iterate for the length of the deck
  for (var i = 0; i < deck.length; i++) {
    //generate a random number from 1 to the number of unshuffled strings
    random = Math.floor(Math.random() * unshuffled);
    //splice that index from the deck
    shuffling = deck.splice(random, 1)[0];
    //push it onto the back of the deck
    deck.push(shuffling);
    //decrement the number of unshuffled strings
    unshuffled--;
  }
  //return the deck
  return deck;
