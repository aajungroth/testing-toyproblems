...
//output the count of the number of leaf nodes
//input none
//constraints linear time, constant spaace
//edge cases, single nodes return 1
Tree.prototype.countLeaves = function () {
  //declare a counter for leaf nodes
  var leafCount = arguments[0] || 0;

  //if the length of the children array is zero
  if (this.children.length <= 0) {
    //increment the leaf node counter by 1
    leafCount++;
  } else {
    //iterate through the children
    for (var i = 0; i < this.children.length; i++) {
      //call the inner recursive function on the children
      leafCount = this.children[i].countLeaves(leafCount);
    }
  }
